...
export default {
    login: async (req, h) => {
        try {
            let foundUser = await database.user.findOne({ email: req.payload.email,
                                                            password: generateHash(req.payload.password)});
    ...
    addPost: async (request, h) => {
        try {
            ...
            console.log(e);
            return Boom.badRequest('Произошла ошибка при добавлении поста');
        }
    },
    getPosts: async (request, h) => {
        try {
            const result = await database.post.find().sort('-createdAt');
            console.log(result);
            return result;
        } catch (e) {
            console.log(e);
            return Boom.badRequest('Произошла ошибка при загрузке постов');
        }
    },
    ...
    getPost: async (request, h) => {
        try {
            const postId = request.payload.postId;
            const result = await database.post.findOne({
                id: postId
            });
            return result;
        }  catch (e) {
    ...
    search: async (request, h) => {
        try {
            const string = request.payload.search;
            const searchString = new RegExp(string ,'i');
            const result = await database.post.find({
                text: searchString
            });
            return result;
        }  catch (e) {
